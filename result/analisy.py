...
def analisy(data_type, id):
    ...
    sys_tardiness_rate = savgol_filter(
        system_record["tardiness_rate"], window_length=11, polyorder=2
    )
    ...
    plt.plot(
        savgol_filter(np.mean(reward_rl, axis=0), window_length=11, polyorder=2),
        label="RL",
    )
    for k, reward in enumerate(reward_rl, 1):
        plt.plot(
            savgol_filter(reward, window_length=11, polyorder=2),
            alpha=0.2,
            label=f"machine{k}",
        )
    ...
    plt.subplot(322)
    plt.plot(
        savgol_filter(sys_tardiness, window_length=11, polyorder=2),
        label="avg_tardness",
    )
    # plt.plot(
    #     savgol_filter(result, window_length=11, polyorder=2), label="mean_tardiness"
    # )
    plt.legend()
    plt.title("sys_avg_tardness")

    plt.subplot(323)
    plt.title("sys_reward")
    plt.plot(
        savgol_filter(sys_reward, window_length=11, polyorder=2),
        label="sys_reward",
    )
    plt.legend()
    ...
    plt.subplot(324)
    plt.plot(
        savgol_filter(reward_rl.mean(axis=0), window_length=11, polyorder=2),
        label="mean_reward",
    )
    plt.title("mean_reward")
    plt.legend()
    ...
    plt.subplot(325)
    plt.plot(
        savgol_filter(np.sum(tardiness, axis=0), window_length=11, polyorder=2),
        label="sum tardiness",
    )
    plt.legend()
    ...
    plt.plot(
        savgol_filter(actor_loss * 10, window_length=11, polyorder=2),
        label="actor_loss",
    )
    plt.plot(
        savgol_filter(critic_loss * 10, window_length=11, polyorder=2),
        label="critic_loss",
    )
    plt.plot(
        savgol_filter(entropy * 10, window_length=11, polyorder=2), label="entropy"
    )
    plt.legend()

    correction, p_value = pearsonr(np.sum(tardiness, axis=0), reward_rl.mean(axis=0))
    print(
        f"correlation between tardiness and G: {correction:.4f}, p_value: {p_value:.2f}"
    )
    correction, p_value = spearmanr(np.sum(tardiness, axis=0), reward_rl.mean(axis=0))
    print(
        f"correlation between tardiness and G: {correction:.4f}, p_value: {p_value:.2f}"
    )
    plt.tight_layout()
