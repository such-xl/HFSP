...
class Machine(Node):
    ...
    def show(self):
        print(
            f"machine {self._id} status:{self._status} job:{self._job.id} t_process:{self._t_process} t_processed:{self._t_processed}"
        )
    ...
    def load_job(self, job, time_step):
        ...
        self._job = job
        self.draw_data.append(
            [job.id - 1, time_step, time_step + job.get_t_process(self._id)]
        )
        job.load_to_machine(self, time_step)
        self._status = MachineStatus.RUNNING
        self._last_decison_time = time_step
        # print('machine',self._id,'load job',job.id,'time_step',time_step,'t_process',job.get_t_process(self._id))
    ...
    def run(self, min_run_timestep, time_step):
        ...
        self.busy_time += min_run_timestep
