...
def fetch_jobs_from_file(path: str):
    ...
    job_machine_op_matrix = np.zeros((job_num, machine_num, max_op_num + 1), dtype=int) + INF
...
class HFSPEnv(gym.Env):
    def __init__(self, render_mode: Optional[str] = None):
        ...
        self.observation_space = gym.spaces.Box(np.array([0] * 20 + [-INF] * 20, dtype=int),
                                                np.array([20] * 20 + [INF] * 20, dtype=int), dtype=np.int32)
        self.fig = None
    ...
    def reset(
        ...
        self.job_machine_op_matrix, self.job_op_num = fetch_jobs_from_file(train_data_path)
        ...
        for mi in range(self.action_dim):
            ...
            for op in range(self.job_machine_op_matrix.shape[2]):
                self.machine_op_dict[mi][op] = set(
                    [job for job, v in enumerate(self.job_machine_op_matrix[:, mi, op]) if v < INF] + [self.IDlE])

        self.current_job_state = np.zeros((2,) + (self.job_dim,),
                                          dtype=int)  # 0-op id, 1-makespan
        self.current_machine2job_matrix = np.zeros(self.job_machine_op_matrix.shape[:2])  # all idle
        self.current_time = 0
    ...
    def render(self) -> Union[RenderFrame, list[RenderFrame], None]:
        ...
        image = np.zeros((self.job_machine_op_matrix.shape[0], self.job_machine_op_matrix.shape[2], 3))
    ...
    def step(self, action: ActType) -> tuple[ObsType, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        op_running = [(False if a >= self.IDlE else (self.current_job_state[1, a] > 0)) for a in action]
        ...
        if len(available_job_set) > 0:
            ...
            for mi, job in enumerate(action):
                if not idle_machine_tag[mi]:
                    action[mi] = self.IDlE
                    continue
                if self.job_machine_op_matrix[job, mi, self.current_job_state[0, job]] < INF and self.current_job_state[1, job] <= 0:  # compatible
                    continue
                js = set(available_job_list[np.where(self.job_machine_op_matrix[available_job_list, mi, self.current_job_state[0, available_job_list]] < INF)[0]])
        ...
        makespan = self.job_machine_op_matrix[available_actions, available_actions_tag,
        ...
        self.current_machine2job_matrix[available_actions, available_actions_tag] = available_actions
        self.current_time += 1
        self.current_job_state[1, :] -= 1
        self.current_machine2job_matrix[self.current_job_state[1, :] <= 0, :] *= 0

        # op is completed ?
        if all(self.current_job_state[1, :] <= 0 & np.greater_equal(self.current_job_state[0, :], self.job_op_num)):
            terminated = True
