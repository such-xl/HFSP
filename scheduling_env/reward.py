...
class AsyncMachineUtilizationReward:
    ...
    def update_system_state(self, force=False):
        ...
        # 检查是否有足够的数据更新系统状态
        if all(len(self.machine_history[i]['utilization']) > 0 for i in range(self.num_machines)):
            # 获取每台机器的最新利用率
    ...
    def get_time_weighted_utilization(self, machine_id, current_time=None):
        ...
        # 计算加权利用率
        weighted_utilization = np.sum(
            np.array(list(history['utilization'])) * weights
        )
    ...
    def calculate_machine_reward(self, machine_id, current_time=None):
        ...
        # 获取机器的时间加权利用率
        machine_utilization = self.get_time_weighted_utilization(machine_id, current_time)
    ...
    def calculate_system_reward(self, current_time=None):
        ...
        # 计算系统奖励
        system_reward = self.w1 * system_mean_utilization - self.w2 * system_std_utilization
...
def example_async_usage():
    ...
    for t in range(20):
        ...
        for machine_id in range(num_machines):
            ...
            # 更新机器利用率
            reward_calculator.update_machine_utilization(machine_id, utilization, current_time)
            ...
            print(f"  局部奖励: {details['local_reward']:.4f}, 全局奖励: {details['global_reward']:.4f}")
    ...
    for episode in range(5):
        ...
        for step in range(100):
            ...
            for machine_id in range(num_machines):
                ...
                # 模拟执行动作后的利用率变化
                utilization = np.random.uniform(0.4, 0.9)  # 这应该是动作执行后的实际利用率
            ...
            if step % 50 == 0:
                ...
                print(f"Step {step}, System Reward: {system_reward:.4f}, Mean Utilization: {details['system_mean_utilization']:.4f}")
