...
class PPO:
    def __init__(
        ...
        self.old_actor = Actor(local_state_dim*local_state_len, act_dim,).to(device)
    ...
    def store_transition(self, local_state, global_state, action, reward, next_local_state, next_global_state, done):
    ...
    def update(self):
        ...
        with torch.no_grad():
         ...
         # 计算 两个目标 TD 目标值 
        rewards_1 = (rewards[:,0] - rewards[:,0].mean()) / (rewards[:,0].std() + 1e-8)
        rewards_2 = (rewards[:,1] - rewards[:,1].mean()) / (rewards[:,1].std() + 1e-8)   
...
#         objs = np.array(self.obj_record)
#         v_max, v_min = objs.max(axis=0), objs.min(axis=0)
#         norm_objs = (objs - v_min) / (v_max - v_min + 1e-6)
#         pareto = get_ps(norm_objs)
#         return {
#             'pareto': pareto,
#             'IGD': IGD(pareto, norm_objs),
#             'GD': GD(norm_objs, pareto),
#             'Spread': spread(norm_objs, pareto)
#         }
