...
class AsyncTardinessReward:
    ...
    def normalize_reward(self, reward, scale=0.01):
        ...
        normalized = np.tanh(scale*reward)
        return normalized
    ...
    def calculate_slack_change(self, machine_id, assigned_job_id, current_time, processing_time):
        """
        计算因为机器决策导致的松弛时间变化
        ...
        # 保存决策前的系统状态
        pre_decision_slacks = {j_id: info['slack'] for j_id, info in self.job_tracking.items()}
        ...
        assigned_job['slack'] = assigned_job['deadline'] - (current_time + processing_time) - assigned_job['remaining_time']
        
        # 更新其他作业的松弛时间（它们可能因为等待而减少）
        for job_id, job_info in self.job_tracking.items():
            if job_id != assigned_job_id:
                # 假设该作业可能需要等待processing_time
                job_info['slack'] = job_info['deadline'] - (current_time + processing_time) - job_info['remaining_time']
        ...
        if other_jobs:
            other_slack_changes = [self.job_tracking[j_id]['slack'] - pre_decision_slacks[j_id] for j_id in other_jobs]
    ...
    def update_system_state(self, current_time, force=False):
        ...
        if self.system_history['total_jobs'] > 0:
            tardiness_rate = self.system_history['tardy_jobs'] / self.system_history['total_jobs']
    ...
    def calculate_urgency_factor(self, job_id):
        ...
        # 使用指数衰减函数计算紧急度
        urgency = np.exp(-self.urgency_factor * slack / max(1, self.job_tracking[job_id]['remaining_time']))
        return urgency
    
    def calculate_machine_reward(self, machine_id, assigned_job_id, current_time, processing_time):
        """
        计算单个机器的决策奖励
    ...
    def calculate_system_reward(self, current_time):
        ...
        if self.system_history['total_jobs'] > 0:
            ...
            if self.system_history['tardy_jobs'] > 0:
                avg_tardiness = self.system_history['total_tardiness'] / self.system_history['tardy_jobs']
    ...
    def get_machine_contribution(self, machine_id, window=10):
        ...
        # 计算非迟到作业比例
        non_tardy_rate = sum(1 for job in recent_jobs if not job['is_tardy']) / len(recent_jobs)
        
        # 计算平均迟到时间
        avg_tardiness = np.mean([job['tardiness'] for job in recent_jobs]) if recent_jobs else 0
