...
class D3QNAgent:
    def __init__(self, train_params, model_params) -> None:
        ...
        self.main_net = D3QN(
            state_dim=model_params["state_dim"], action_dim=model_params["action_dim"]
        ).to(self.device)
        ...
        self.optimizer = torch.optim.AdamW(
            self.main_net.parameters(), lr=train_params["learning_rate"]
        )
    ...
    def take_action(
        ...
        ) * np.exp(-1.0 * step_done / self.epsilon_decay)
    ...
    def update(self, transition):
        ...
        with torch.no_grad():
            ...
            next_q_values = next_q_values.gather(1, max_actions.unsqueeze(-1)).squeeze(
                -1
            )
        ...
        for key in actor_state_dict:
            target_state_dict[key] = actor_state_dict[
                key
...
class SACAgent:
    def __init__(self, model_params, train_params) -> None:
        ...
        self.q1_optimizer = torch.optim.AdamW(
            self.q1.parameters(), lr=train_params["learning_rate"]
        )
        self.q2_optimizer = torch.optim.AdamW(
            self.q2.parameters(), lr=train_params["learning_rate"]
        )

        self.policy_optimizer = torch.optim.AdamW(
            self.policy.parameters(), lr=train_params["learning_rate"]
        )
    ...
    def take_action(self, state, deterministic=False):
        with torch.no_grad():
            ...
            action = (
                beta.rvs(alpha.detach().numpy(), beta.detach().numpy()).cpu().item()
            )
            return action
    ...
    def update(self, transition):
        ...
        policy_loss = (
            (probs * (self.alpha * torch.log(probs + 1e-9) - q_values))
            .sum(dim=-1)
            .mean()
        )
...
class Agents:
    def __init__(self, train_params, model_params) -> None:
        self.agents = [
            SACAgent(model_params, train_params)
            for _ in range(model_params["machine_seq_len"])
        ]
        self.tau = train_params["tau"]
        self.device = train_params["device"]
        self.gamma = train_params["gamma"]
    ...
    def take_actions(self, state) -> list:
        ...
        for i, agent in enumerate(self.agents):
            ...
            machine_state = torch.as_tensor(machine_state).to(
    ...
    def get_batch_actions(self, states):
        actions = torch.zeros(
            [states.size(0), len(self.agents)], dtype=torch.int64, device=self.device
        )
        for i, agent in enumerate(self.agents):
            machine_state = [[[1 if i == j else 0 for j in range(16)]]]
            machine_state = torch.as_tensor(machine_state).to(
    ...
    def update(self, transition):
        ...
        with torch.no_grad():
            next_actions = self.get_batch_actions(next_states).to(
                dtype=torch.int64, device=self.agents[0].device
            )
