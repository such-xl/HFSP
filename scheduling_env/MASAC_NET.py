...
class Agent:
    ...
    def take_action(
        ...
        if np.random.rand() < self.eps_threshold:
            ...
            with torch.no_grad():
                ...
                machine_state = (
                    torch.as_tensor(machine_state).to(self.device).unsqueeze(0)
                )
                mem = self.main_net.encoder(state)
                q_value = self.main_net(mem, machine_state).squeeze(0)
                action_mask_copy = np.copy(action_mask)
                action_mask_copy = torch.as_tensor(
                    action_mask_copy, dtype=torch.bool
                ).to(self.device)
    ...
    def update(self, transition):
        ...
        Q_all = torch.zeros(
            machine_state.size(0), machine_state.size(1), device=q_values.device
        )
