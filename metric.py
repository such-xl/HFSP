...
# 方法列表
METHODS = ["RL", "SPT", "LPT", "LRPT", "Random", "FIFO"]
...
def eval_model(env: TrainingEnv, method: str):
    ...
    while not done:
        if method == "RL":
            ...
            idx = {"SPT": 0, "LPT": 1, "LRPT": 2, "Random": 3, "FIFO": 4}[method]
            _, done, _ = env.sr(idx)
...
def save_to_excel(save_dir, gd_vals, igd_vals, spr_vals):
    save_path = save_dir + "evaluation.xlsx"
    ...
    columns_extended = ["M", "E", "J"] + METHODS
    # 构造新的数据
    gd_df = pd.DataFrame([[machine_num,E_ave,new_insert] + gd_vals], columns=columns_extended)
    igd_df = pd.DataFrame([[machine_num,E_ave,new_insert] + igd_vals], columns=columns_extended)
    spread_df = pd.DataFrame([[machine_num,E_ave,new_insert] + spr_vals], columns=columns_extended)
