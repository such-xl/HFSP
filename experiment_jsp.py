...
def eval_mappo(env: FJSEvalEnv, mappo: AsyncMAPPO):
    ...
    while not done and not truncated:
        ...
        next_obs, next_obs_mask, _, _, done, truncated = env.step(action)
        obs_i = next_obs
...
if __name__ == "__main__":
    ...
    for ur in PARAMS["UR"]:
        ...
        for job in jobs_name:
            ...
            tards_record = eval_mappo(env_rl, mappo)
            print(f"UR:{ur} {job} total tard:{np.sum(tards_record)}")
    ...
    for ur in PARAMS["UR"]:
        ...
        for sr in PARAMS["SR"]:
            for job in jobs_name:
                ...
                tards_record = eval_sr(env_sr, sr)
