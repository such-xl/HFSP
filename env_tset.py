...
from scheduling_env.basic_scheduling_algorithms import noname_2,EDD,MS,SRO,CR
np.random.seed(42)
EVAL_EPISODE = 10

METHODS = ["RL", "EDD", "MS", "SRO", "CR"]
...
# mappo.load_model()
record = {method: {"tard": [], "slack": [], "util": [],"makespan":[]} for method in METHODS}
    ...
    for episode in range(1):
        ...
        while not (done or truncted):
            if method == "RL":
                ...
                _,done,truncted = env.step_by_sr(action)
            elif method == "SRO":
...
if __name__ == "__main__":
    ...
    for method in METHODS:
        ...
        print(f"{method:}\t {tard:.2f}\t{util_mean:.4f}\t{util_std:.4f}\t{slack:.2f}\t{makespan:.2f}")
